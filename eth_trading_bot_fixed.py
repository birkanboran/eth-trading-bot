#!/usr/bin/env python3
"""
ETH/USDT Perpetual Trading Bot - Swing High/Low Strategy (FIXED)
- No duplicate signals (proper state tracking)
- Position close information in SELL signal
- Correct timezone (UTC)
- 5x leverage, 1% risk per trade
"""
...
# Setup logging with UTC timezone
logging.basicConfig(
    level=logging.INFO,
    format='[%(asctime)s UTC] %(levelname)s: %(message)s',
    handlers=[
        logging.FileHandler('/home/ubuntu/bot_fixed.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

class SwingTradingBotFixed:
    def __init__(self):
        ...
        # State tracking - CRITICAL for preventing duplicate signals
        self.last_signal = None
        self.last_signal_index = None  # Track which candle generated the signal
        ...
        self.stats_file = '/home/ubuntu/bot_fixed_stats.json'
        self.trades_file = '/home/ubuntu/bot_fixed_trades.json'
        self.load_stats()
        
        logger.info("Bot initialized - Swing High/Low Strategy (5x leverage, FIXED)")
        ...
        if os.path.exists(self.stats_file):
            try:
                with open(self.stats_file, 'r') as f:
                    ...
                    self.last_signal = stats.get('last_signal', None)
                    self.last_signal_index = stats.get('last_signal_index', None)
    ...
    def generate_signal(self, df: pd.DataFrame) -> Optional[Dict]:
        """Generate trading signal at swing points (NO DUPLICATES)"""
        if len(df) < 10:
            return None
        ...
        # CRITICAL: Check if this is a NEW signal (different index)
        if self.last_signal_index == last_swing_idx:
            # Same signal index = duplicate, skip
            return None
...
🎯 TP: ${signal_data['tp_price']:.2f} (+2%)
🛑 SL: ${signal_data['sl_price']:.2f} (-2%)
...
🕐 {signal_data['timestamp']}
            ...
            self.last_signal_index = signal_data['signal_index']
            self.save_stats()
...
{signal_data['close_reason']} Closed by: {signal_data['close_reason'].split()[0]}

📊 Position Size: {signal_data['position_size']:.4f} ETH
⚡ Leverage: {signal_data['leverage']}x

💵 New Balance: ${signal_data['balance']}
📈 Total Trades: {self.trade_count}
📊 Daily PnL: ${self.daily_pnl:.2f}
📊 Weekly PnL: ${self.weekly_pnl:.2f}
📊 Monthly PnL: ${self.monthly_pnl:.2f}
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                # Log current state
                current = df.iloc[-1]
                logger.info(f"Price: ${current['close']:.2f} | Balance: ${self.balance:.2f} | Trades: {self.trade_count}")
